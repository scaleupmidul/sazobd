...
const WomenPage: React.FC = () => {
    ...
    const categories = useMemo(() => {
        ...
        return [
            { name: 'All', label: 'All Styles' },
            ...cats.map(c => ({ name: c, label: c }))
        ];
    }, [settings.categories]);
            ...
            <main id="women-grid" className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-12">
                <div className="flex justify-between items-end mb-10 px-2">
                    <div>
                        <h2 className="text-2xl sm:text-4xl font-extrabold text-stone-900">
                           {activeFilter === 'All' ? "Women's Fashion" : activeFilter}
                        </h2>
                ...
                {womenProducts.length === 0 && !loading && (
                    <div className="py-20 text-center">
                        ...
                        <button onClick={() => setActiveFilter('All')} className="mt-4 text-pink-600 font-bold border-b border-pink-600">Show All Styles</button>
                    </div>
                )}
            </main>
