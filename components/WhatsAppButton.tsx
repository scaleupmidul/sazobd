
import React from 'react';
import { useAppStore } from '../StoreContext';

const MessageIcon = () => (
    <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="currentColor"
        className="w-9 h-9"
    >
        <path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z" />
    </svg>
);


const WhatsAppButton: React.FC = () => {
    const { settings } = useAppStore();

    if (!settings.whatsappNumber || settings.whatsappNumber.trim() === '') {
        return null;
    }

    // Clean the number: remove all non-digit characters
    const cleanPhoneNumber = settings.whatsappNumber.replace(/\D/g, '');
    const whatsappUrl = `https://wa.me/${cleanPhoneNumber}`;

    return (
        <a
            href={whatsappUrl}
            target="_blank"
            rel="noopener noreferrer"
            aria-label="Chat on WhatsApp"
            className="group fixed bottom-8 right-6 z-50 flex items-center"
        >
            {/* Text Bubble */}
            <div className="absolute right-full mr-3 whitespace-nowrap rounded-lg bg-white px-4 py-2 text-sm font-semibold text-pink-600 shadow-lg opacity-0 transition-all duration-300 ease-in-out group-hover:opacity-100 transform scale-95 group-hover:scale-100 origin-right">
                Do you want to know anything!
            </div>
            
            {/* Pink Button */}
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-pink-600 text-white shadow-xl transition-all duration-300 ease-in-out group-hover:scale-105 group-hover:shadow-2xl animate-pulse-whatsapp">
                <MessageIcon />
            </div>
        </a>
    );
};

export default WhatsAppButton;